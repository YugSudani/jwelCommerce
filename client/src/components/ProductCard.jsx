    ...
    >
      ...
      <div className="p-3 sm:p-6 flex flex-col flex-1">
        <Link to={`/product/${product._id}`}>
          <h3 className="text-sm sm:text-xl font-bold text-gray-900 dark:text-white mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
            {product.name}
          </h3>
        </Link>
        ...
        {/* Price + Button */}
        <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/50 -mx-3 sm:-mx-6 -mb-3 sm:-mb-6 px-3 sm:px-6 py-2.5 sm:py-4 mt-auto border-t border-gray-100 dark:border-white/5">
          <span className="text-base sm:text-2xl font-black text-blue-600 dark:text-blue-400 leading-none">
            ₹{product.price}
          </span>
          <Link
            to={`/product/${product._id}`}
            className="flex items-center gap-1 sm:gap-2 bg-blue-600 hover:bg-blue-700 text-white px-2.5 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm transition-all active:scale-95"
          >
            Details
